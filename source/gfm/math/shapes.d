...
struct Plane(T) if (isFloatingPoint!T)
{
    public
    {
        tvec3!T n; /// Normal (always stored normalized).
        T d;

        /// Create from four coordinates.
        @nogc this(tvec4!T abcd) pure nothrow
        {
            n = tvec3!T(abcd.x, abcd.y, abcd.z).normalized();
            d = abcd.w;
        }

        /// Create from a point and a normal.
        @nogc this(tvec3!T origin, tvec3!T normal) pure nothrow
        {
            n = normal.normalized();
            d = -dot(origin, n);
        }
        ...
        @nogc this(tvec3!T A, tvec3!T B, tvec3!T C) pure nothrow
        {
...
struct Frustum(T) if (isFloatingPoint!T)
{
    public
    {
        ...
        @nogc bool contains(tvec3!T point) pure const nothrow
        {
            for(int i = 0; i < 6; ++i)
            {
            ...
            for (int i = 0; i < 2; ++i)
                for (int j = 0; j < 2; ++j)
                    for (int k = 0; k < 2; ++k)
                    {
                        ...
                        corners[i*4 + j*2 + k] = tvec3!T(x, y, z);
                    }
